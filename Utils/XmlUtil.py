...
#    document = dom.
#    return document
def createSketchRoot(dipCalculator,   name, rectView, colorWriter) :
    ...
    element.set("xmlns:app","http://schemas.android.com/apk/res-auto")
#    
...
def addAttributeColor(element, attributeName, color, colorWriter) :
    colorId = colorWriter.addResource(selectColorMode(color))
    element.set(attributeName, getReferenceColorId(colorId))

def addAttribute(element, attributeName, attributeValue) :
...
def	 getArrayReferenceResourceId(_id) :
		return "@" + Constants.VALUE_ARRAY_FILE_NAME + "/" + str(_id)
...
def	  addAdditionalAttribute(element,  attributeName, attributeValue) :
    element.set(attributeName, attributeValue)
