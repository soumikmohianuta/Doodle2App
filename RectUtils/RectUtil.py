...
# Utls used for rects
def fixInnerIntersection(parent, child):
    rx = child.x
    ry = child.y
    rh = child.height
    rw = child.width
    ...
    if(rx+rw>tx+tw):
        child.width = child.width- (rx+rw-tx-tw)-2
            
    if(ry+rh>ty+th):
        print(ry+rh-ty-th)
        child.height = child.height- (ry+rh-ty-th)-2
...
def outerIntersection(parent,child,width,height):
    rx = child.x
    ry = child.y
    rh = child.height
    rw = child.width
    ...
    if(rx<tx and rx+rw>tx):
        child.width = tx-rx-2
    
    if(ry<ty and ry+rh>ty):
        child.height = ty-ry-2

    if(rx<tx+tw and rx+rw>tx+tw):
        child.x = tx+tw+2
        if(child.x+child.width>width):
            child.width = width-child.x
#
    if(ry<ty+th and ry+rh>ty+th):
        child.y = ty+th+2
        if(child.y+child.height>height):
            child.height = height-child.y
            
    return child

def fixOverlap(parent,child,width,height ):
    rx = child.x
    ry = child.y
    rh = child.height
    rw = child.width
...
#        print("Element within other element")

        return child ,False, False

#   intersect        
    intersection = interection(parent, child)
    ratio= float( intersection.area()) / child.area()
#   if fall within container
    if(ratio >= .5):

        return child ,False, False
    else:
        child = outerIntersection(parent,child, width, height)

        return child ,True, False

def fixHierarchy(parent,child,width,height ):
    rx = child.x
    ry = child.y
    rh = child.height
    rw = child.width
...
#        print("item is parent")
        return child, True, False
#    Upper or left
    if (rx < tx and rx+rw<tx) or (ry < ty and ry+rh<ty):
#        print("element upper or left")
        return child ,True, False 

#    Down or right
    if (rx > tx+tw or ry > ty+th ):
#        print("element down or right")

        return child ,True, False

    if(not parent.isContainer()):
        return fixOverlap(parent,child,width,height )

#   its within
    if (rx > tx and ry > ty and  rx+rw<tx+tw and ry+rh<ty+th):
        child.x = child.x-parent.x
        child.y = child.y - parent.y
#        print("item is within parent")
...
#   if fall within container
    if(ratio >= .5):
#        print("Fixing  Container Inner Overlap")
        print(child.height)

        child = fixInnerIntersection(parent,child)
        print(child.height)

        return child , True, True

    else:
#        print("Fixing  Container Outer Overlap")
